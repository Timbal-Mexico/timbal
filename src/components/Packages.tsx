...
const Packages = () => {
  ...
  const totalGroups = Math.ceil(packages.length / itemsPerView);
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            <div className="relative">
              ...
              <button
                onClick={handlePrev}
                className="absolute left-0 top-1/2 -translate-y-1/2 z-10 p-3 rounded-full bg-primary text-primary-foreground hover:opacity-80 transition-opacity hover:scale-110 active:scale-95"
              >
                <ChevronLeft className="w-6 h-6" />
              </button>

              {/* Slider Grid with Slide Animation */}
              <AnimatePresence initial={false} custom={direction} mode="wait">
                <motion.div
                  ...
                  className="grid grid-cols-1 lg:grid-cols-3 gap-6 px-16"
                >
              ...
              <button
                onClick={handleNext}
                className="absolute right-0 top-1/2 -translate-y-1/2 z-10 p-3 rounded-full bg-primary text-primary-foreground hover:opacity-80 transition-opacity hover:scale-110 active:scale-95"
              >
            ...
            {Array.from({ length: totalGroups }).map((_, index) => (
              <motion.button
                ...
                className={`rounded-full transition-all ${
                  currentIndex === index
                    ? "bg-primary w-8 h-3"
                    : "bg-muted hover:bg-muted-foreground w-3 h-3"
                }`}
      ...
      <div className="container mx-auto px-6">
        <div className="max-w-5xl mx-auto">
          ...
          >
            ...
            <p className="text-lg text-muted-foreground max-w-3xl mx-auto">
              Elige la solución que tu empresa necesita. Todos nuestros paquetes incluyen diagnóstico, implementación y capacitación.
            </p>
          </motion.div>

          {/* Packages Grid */}
          <div className="space-y-6 mb-12">
            {/* Primera fila - 2 paquetes */}
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <motion.div
                ...
                className={`relative rounded-2xl border transition-all hover:shadow-lg p-8 ${pkg.borderColor} ${pkg.bgColor} ${
                  pkg.featured ? "md:col-span-2 lg:col-span-2" : ""
                }`}
              >
                {/* Featured Badge */}
                {pkg.featured && (
                ...
                {/* Package Header */}
                <div className={`mb-6 ${pkg.featured ? "mt-4" : ""}`}>
                  <div className={`w-12 h-12 rounded-xl bg-gradient-to-r ${pkg.color} flex items-center justify-center mb-4`}>
                    <pkg.icon className="text-white w-6 h-6" />
                  </div>
                  <h3 className="font-display text-2xl font-bold text-foreground mb-1">
                    {pkg.name}
                  </h3>
                  <p className="text-sm text-muted-foreground mb-4">{pkg.subtitle}</p>
                  <p className="text-foreground mb-6 italic">{pkg.description}</p>
                  <p className={`text-sm font-semibold mb-6 p-3 rounded-lg bg-gradient-to-r ${pkg.color} text-white`}>
                    "{pkg.tagline}"
                  </p>
                </div>
                ...
                {/* Pricing */}
                <div className="mb-8 p-4 rounded-lg border border-border/50 bg-muted/20">
                  <h4 className="font-semibold text-foreground mb-4">Inversión:</h4>
                  <div className="space-y-2">
                    {pkg.pricing.map((price, idx) => (
                      <div key={idx} className="flex justify-between items-center text-sm">
                        <span className="text-muted-foreground">{price.users}</span>
                  ...
                  <div className="border-t border-border/50 mt-4 pt-4">
                    <p className="text-xs text-muted-foreground">
                      <span className="font-semibold">Mantenimiento anual:</span> {pkg.maintenance}
                    </p>
                  </div>
                </div>
            ...
            {/* Tercera fila - Full Control (destacado) */}
            <motion.div
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.5, delay: 0.4 }}
              className={`relative rounded-2xl border transition-all hover:shadow-lg p-8 ${packages[4].borderColor} ${packages[4].bgColor} md:col-span-2`}
            >
              {/* Featured Badge */}
              <div className="absolute top-6 left-6">
                <span className="inline-block px-4 py-1 bg-gradient-to-r from-yellow-500 to-amber-600 text-white text-xs font-bold rounded-full">
              ...
              <div className="mb-6 mt-4">
                <div className={`w-12 h-12 rounded-xl bg-gradient-to-r ${packages[4].color} flex items-center justify-center mb-4`}>
                  <IconRenderer icon={packages[4].icon}  className="text-white w-6 h-6" />
                </div>
                <h3 className="font-display text-2xl font-bold text-foreground mb-1">
                  {packages[4].name}
                </h3>
                <p className="text-sm text-muted-foreground mb-4">{packages[4].subtitle}</p>
                <p className="text-foreground mb-6 italic">{packages[4].description}</p>
                <p className={`text-sm font-semibold mb-6 p-3 rounded-lg bg-gradient-to-r ${packages[4].color} text-white`}>
                  "{packages[4].tagline}"
                </p>
              </div>
              ...
              <div className="mb-8">
                <h4 className="font-semibold text-foreground mb-4">Incluye:</h4>
                <ul className="space-y-3">
                  {packages[4].features.map((feature, idx) => (
                    <li key={idx} className="flex items-start gap-3">
                      <Check className="w-5 h-5 text-primary flex-shrink-0 mt-0.5" />
                      <span className="text-muted-foreground text-sm">{feature}</span>
                    </li>
              ...
              <div className="mb-8 p-4 rounded-lg border border-border/50 bg-muted/20">
                <h4 className="font-semibold text-foreground mb-4">Inversión:</h4>
                <div className="space-y-2">
                  {packages[4].pricing.map((price, idx) => (
                    <div key={idx} className="flex justify-between items-center text-sm">
                      <span className="text-muted-foreground">{price.users}</span>
                      <span className="font-semibold text-foreground">{price.price}</span>
                    </div>
                ...
                <div className="border-t border-border/50 mt-4 pt-4">
                  <p className="text-xs text-muted-foreground">
                    <span className="font-semibold">Mantenimiento anual:</span> {packages[4].maintenance}
                  </p>
                </div>
              </div>
            ...
            {/* Cuarta fila - Director View */}
            <motion.div
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.5, delay: 0.5 }}
              className={`relative rounded-2xl border transition-all hover:shadow-lg p-8 ${packages[5].borderColor} ${packages[5].bgColor} md:col-span-1`}
            >
              {/* Package Header */}
              <div className="mb-6">
                <div className={`w-12 h-12 rounded-xl bg-gradient-to-r ${packages[5].color} flex items-center justify-center mb-4`}>
                  <IconRenderer icon={packages[5].icon} className="text-white w-6 h-6" />
                </div>
                <h3 className="font-display text-2xl font-bold text-foreground mb-1">
                  {packages[5].name}
                </h3>
                <p className="text-sm text-muted-foreground mb-4">{packages[5].subtitle}</p>
                <p className="text-foreground mb-6 italic">{packages[5].description}</p>
                <p className={`text-sm font-semibold mb-6 p-3 rounded-lg bg-gradient-to-r ${packages[5].color} text-white`}>
                  "{packages[5].tagline}"
                </p>
              </div>
              ...
              <div className="mb-8">
                <h4 className="font-semibold text-foreground mb-4">Incluye:</h4>
                <ul className="space-y-3">
                  {packages[5].features.map((feature, idx) => (
                    <li key={idx} className="flex items-start gap-3">
                      <Check className="w-5 h-5 text-primary flex-shrink-0 mt-0.5" />
                      <span className="text-muted-foreground text-sm">{feature}</span>
                    </li>
              ...
              <div className="mb-8 p-4 rounded-lg border border-border/50 bg-muted/20">
                <h4 className="font-semibold text-foreground mb-4">Inversión:</h4>
                <div className="space-y-2">
                  {packages[5].pricing.map((price, idx) => (
                    <div key={idx} className="flex justify-between items-center text-sm">
                      <span className="text-muted-foreground">{price.users}</span>
                      <span className="font-semibold text-foreground">{price.price}</span>
                    </div>
                ...
                <div className="border-t border-border/50 mt-4 pt-4">
                  <p className="text-xs text-muted-foreground">
                    <span className="font-semibold">Mantenimiento anual:</span> {packages[5].maintenance}
                  </p>
                </div>
              </div>
          ...
          >
            ...
            <p className="text-muted-foreground mb-6">
              Agenda un diagnóstico gratuito. Analizamos tu proceso actual y recomendamos el paquete ideal para ti.
            </p>
            <Button
              onClick={openCalendly}
              className="gradient-hero text-primary-foreground border-0 hover:opacity-90 transition-opacity"
