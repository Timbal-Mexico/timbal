...
import { Check, X, ArrowRight, Zap, Star, ShieldCheck, ShoppingCart, Package } from "lucide-react";
...
const plans = [
  ...
  },
  {
    ...
    description: "Implementación básica de tienda online profesional para empezar a vender.",
    price: 12000,
    features: [
      "Tienda online profesional",
      "Configuración de pagos",
      "Configuración de envíos",
      "Estructura base para publicar productos",
      "Capacitación básica para administrar la tienda"
    ],
    notIncluded: [],
    cta: "Elegir Ecommerce Starter",
    highlight: false,
    icon: ShoppingCart
  },
  /*
  {
    name: "Ecommerce Starter Plus",
    description: "Tienda Shopify + base de CRM Kommo para tener control y seguimiento desde el inicio.",
    price: 25000,
    features: [
      "Todo lo de Ecommerce Starter",
      "Configuración inicial de Kommo",
      "Diseño de 1 embudo de ventas",
      "Automatización básica de seguimiento",
      "6 horas de capacitación"
    ],
    notIncluded: [],
    cta: "Elegir Ecommerce Starter Plus",
    highlight: false,
    icon: Package
  },
  */
...
const Pricing = () => {
  ...
  type KommoPlanType = (typeof kommoPlansData)[number]["name"];
  const [kommoPlanType, setKommoPlanType] = useState<KommoPlanType>("Avanzado");
  ...
  const ecommerceSectionsByPlan: Record<
    string,
    Array<{
      value: string;
      title: string;
  ...
  > = {
    "Ecommerce Starter": [
      {
        value: "tienda",
        title: "Tienda en Shopify",
        items: [
          "Configuración inicial de la tienda",
          "Plantilla profesional estándar",
          "Configuración de métodos de pago",
          "Configuración básica de envíos",
          "Estructura base para publicar productos"
        ]
    ...
    "Ecommerce Starter Plus": [
      ...
      },
      {
        ...
        items: [
          "6 horas de capacitación",
          "Uso de tienda y flujo operativo base",
          "Uso de CRM para seguimiento"
        ]
      }
    ],
    */
    "Ecommerce 360 Starter": [
      {
        value: "tienda",
        title: "Tienda en Shopify",
        items: [
    ...
    <section id="pricing" className="py-24 bg-background relative">
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="max-w-[90rem] mx-auto">
          ...
          >
            <Card className="h-full flex flex-col border-primary/50 shadow-2xl bg-purple-500/5 transition-all duration-300 hover:bg-purple-500/10 hover:shadow-purple-500/20">
               <CardHeader className="pb-4">
                 ...
                 <CardTitle className="text-2xl font-bold">Kommo CRM</CardTitle>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-3 gap-1 bg-muted/50 p-1 rounded-lg">
                        {kommoPlansData.map((p) => (
                          <button
                            key={p.name}
                            onClick={() => setKommoPlanType(p.name)}
                            className={`text-xs py-2 rounded-md transition-all font-semibold border ${
                        ...
                        <div className="grid grid-cols-4 gap-2">
                          {[6, 9, 12, 24].map((m) => (
                            <button
                              ...
                              className={`text-xs py-2 border rounded-md transition-all ${
                                kommoMonths === m
                      ...
                      {kommoUsers > 10 ? (
                        ...
                        <>
                          ...
                          <div className="text-3xl font-bold text-foreground">
                            {currency === "USD"
                              ? `$${getKommoPrice().toLocaleString()}`
                              : formatMoney(getKommoPrice(), currency).replace(currency === "EUR" ? "€" : "$", currency === "EUR" ? "€" : "$")}{" "}
                            <span className="text-sm font-normal text-muted-foreground">{currency}</span>
                          </div>
                          <div className="text-[10px] text-muted-foreground mt-1">
                            {currency === "USD"
                              ? `$${getKommoPricePerUser()}`
                              : formatMoney(getKommoPricePerUser(), currency).replace(currency === "EUR" ? "€" : "$", currency === "EUR" ? "€" : "$")}{" "}
                            {currency} / usuario / mes
                          </div>
                        </>
                      )}
                    </div>
          ...
          <div className="-mx-6 px-6 lg:mx-0 lg:px-0">
            <div className="flex gap-6 overflow-x-auto pb-2 snap-x snap-mandatory lg:grid lg:grid-cols-4 lg:gap-8 lg:overflow-visible">
              {plans.map((plan, index) => (
                <motion.div
                  ...
                  className="relative h-full group flex-shrink-0 snap-start min-w-[280px] sm:min-w-[320px] lg:min-w-0"
                >
                  {plan.highlight && (
                  ...
                  {plan.name === "Ecommerce 360 Starter" && (
                    ...
                    <CardContent className="flex-grow">
                      ...
                      {plan.name.includes("Ecommerce") ? (
                        <Accordion type="single" collapsible className="w-full">
                          {(ecommerceSectionsByPlan[plan.name] ?? []).map((section) => (
